...
{
public:
    StringWidget(int maxFret, QWidget * p):
        QWidget(p)
    {
...
{
    assert(musicToneFor({0, 0}) == interval.min &&
           musicToneFor({maxString(), maxFret()}) == interval.max);
    ...
    {
        auto w = new StringWidget(maxFret(), this);
        m_strings.append(w);
        layout->addWidget(w);
    }
...
void GuitarEdgeSettingsDialog::setHarmony(const Harmony& harmony)
{
    ...
    {
        auto pos = findFretboardPos(tone, vacantStringsNumb, maxFret());
